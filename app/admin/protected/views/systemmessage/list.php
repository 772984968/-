...
<div class="dux-tools">
	<div class="tools-function clearfix">
		...
		<?php if(in_array('index_activity_add', $power)):?>
			<a class="button button-small bg-dot icon-plus dropdown-toggle" href="<?php echo Url::toRoute(yii::$app->params['url']['index_activity_add']);?>">添加</a>
			<?php endif; ?>
		</div>
	</div>
</div>
<div class="admin-main">
    <div class="panel dux-box">
		<div class="table-tools clearfix">
			<div class="float-left">
				<?php echo html::beginForm(Url::toRoute(yii::$app->params['url']['index_activity']), 'GET');?>
						...
						<div class="field">
							<select name="search[cat_id]" id="cat_id" class="input">
								...
								<?php foreach ($category as $k => $v):?>
									<option value="<?php echo isset($v['type']) ? $v['type'] : '';?>" <?php if (isset($searchvalue['cat_id']) && $searchvalue['cat_id'] == $v['type'])  echo "selected"?>>
					...
					<?php if(is_array($data)&&!empty($data)) :?>
						<?php foreach($data as $v) :?>
							<tr>
								...
								<td><?= Html::encode($v->er_activitytype->name)?></td>
								<td><?= Html::encode($v->url)?></td>
								<td><?= Html::encode(date('Y-m-d H:i:s',$v->create_time))?></td>
								<td><?= Html::encode($v['listorder'])?></td>
								<td>
									<?php if(in_array('index_activity_update', $power)):?>
										<a class="button bg-blue button-small icon-pencil" href="<?php echo Url::toRoute(yii::$app->params['url']['index_activity_update']);?>?id=<?= Html::encode($v->id)?>" title="修改"></a>
									<?php endif; ?>
									<?php if(in_array('index_activity_del', $power)):?>
									 	<a class="button bg-red button-small icon-trash-o js-del"  href="javascript:;"  url="<?php echo Url::toRoute(yii::$app->params['url']['index_activity_del']);?>?isCsrf=0" data="<?= Html::encode($v->id)?>" title="删除"></a>
									<?php endif; ?>
								</td>
							</tr>
						<?php endforeach;?>
