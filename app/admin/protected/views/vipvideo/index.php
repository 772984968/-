...
<div class="dux-tools">
    <div class="tools-function clearfix">
        ...
        <?php if(!empty($config['addUrl'])): ?>
            ...
            <div class="button-group float-right">
                <a class="button button-small bg-dot icon-plus dropdown-toggle" href="<?php echo Url::toRoute(yii::$app->params['url']['VipVideoMAdd']);?>"> 批量添加</a>
            </div>
        <?php endif; ?>
                    ...
                    <?php foreach($data as $v) :?>
                        <tr  class="trshow">
                            ...
                            <?php if(!empty($config['chgUrl'])): ?>
                                <!-- update -->
                                 <a class="button bg-blue button-small icon-pencil" href="<?php echo Url::toRoute(yii::$app->params['url'][$config['chgUrl']]);?>?id=<?= Html::encode($v['id'])?>" title="修改"></a>
                            <?php endif; ?>
                            <?php if(!empty($config['delUrl'])): ?>
                                <!-- delete -->
                                <a class="button bg-red button-small icon-trash-o js-del"  href="javascript:;"  url="<?php echo Url::toRoute(yii::$app->params['url'][$config['delUrl']]);?>?isCsrf=0" data="<?= Html::encode($v['id'])?>" title="删除"></a>
                            <?php endif; ?>
                                <!--<a class=""  href="/user/clearmemberone?isCsrf=0&uid=<?/*= Html::encode($v['id'])*/?>"  url=""  title="">消除会员</a>-->
