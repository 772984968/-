...
<div class="dux-tools">
    <div class="tools-function clearfix">
        ...
        <div class="button-group float-right">
            <a class="button button-small bg-dot icon-plus dropdown-toggle" href="<?php echo Url::toRoute(yii::$app->params['url']['apppowerAdd']);?>"> 添加菜单</a>
        </div>
    </div>
</div>
                    ...
                    <?php if(is_array($data)&&!empty($data)) :?>
                        <?php foreach($data as $v) :?>
                            <tr <?php if($v['level'] == '0'): ?> class="trshow" <?php else: ?> style="display:none;" <?php endif; ?> >
                                ...
                                <td>
                                    <!-- update -->
                                     <a class="button bg-blue button-small icon-pencil" href="<?php echo Url::toRoute(yii::$app->params['url']['apppowerEdit']);?>?id=<?= Html::encode($v['id'])?>" title="修改"></a>
                                    <!-- delete -->
                                    <a class="button bg-red button-small icon-trash-o js-del"  href="javascript:;"  url="<?php echo Url::toRoute(yii::$app->params['url']['apppowerDel']);?>?isCsrf=0" data="<?= Html::encode($v['id'])?>" title="删除"></a>
