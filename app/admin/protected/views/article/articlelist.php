					...
					<?php if(is_array($data)&&!empty($data)) :?>
						<?php foreach($data as $v) :?>
							<tr>
								...
								<td><?= Html::encode($v->cate->cat_name)?></td>
								<td><?= Html::encode(date('Y-m-d H:i:s',$v->create_time))?></td>

								<td><?= Html::encode($v['sort_order'])?></td>
								<td>
									<?php if(in_array('editarticle', $power)):?>
										<a class="button bg-blue button-small icon-pencil" href="<?php echo Url::toRoute(yii::$app->params['url']['editarticle']);?>?id=<?= Html::encode($v->id)?>" title="修改"></a>
									<?php endif; ?>
