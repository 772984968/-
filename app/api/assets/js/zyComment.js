		...
		return this.each(function(){
			...
			this.createAgoCommentHtml = function(){
				...
				$.each(para.agoComment, function(k, v){
					...
					var item = '';
					item += '<div class="oun-pl-conter" comment_id="'+v.comment_id+'" target_id="'+v.target_id+'" user_id="'+v.user_id+'">';
					...
					for (var i in v.son)
						{
							if (v.son[i].receiver=="") {
								item += '<p><span class="oun-font cllll" comment_id="'+v.son[i].comment_id+'"  comment2_id="'+v.son[i].comment2_id+'" user_id="'+v.son[i].user_id+'" >'+v.son[i].nickname+':</span><span>&nbsp;'+v.son[i].content+'</span></p>';
	
							}
							else{
								item += '<p><span class="oun-font cllll" comment_id="'+v.son[i].comment_id+'"  comment2_id="'+v.son[i].comment2_id+'" user_id="'+v.son[i].user_id+'" >'+v.son[i].receiver+'</span><span class="oun-font">'+'&nbsp;回复&nbsp;'+v.son[i].nickname+':'+'</span><span>'+v.son[i].content+'</span></p>';
					...
					$("body").on("click",".oun-more .oun-font",function  () {
						$(this).parents(".oun-pl-conter").find(".oun-pl-bottom p:gt(2)").show(200)
...
$("body").on("click",".oun-nron",function  () {
	
	var yhuid=$(this).parents(".oun-pl-conter").attr("user_id")
	var _this=$(this)
//	var a=$(this).parents(".oun-pl-conter").attr("comment_id")

	if (yhuid==userid) {
		$(".scpl-wrap").slideDown(function  () {
			$(".y-qr").click(function  () {
				var CommentForm={};
				
				CommentForm['id']=_this.parents(".oun-pl-conter").attr("comment_id")
				var json = {CommentForm};
				$.post("/product/commentdel?token="+token+"&userid="+userid+"",json,function  (data) {
					
					$(".scpl-wrap").slideUp()
					_this.parents(".oun-pl-conter").remove()
				...
				$(".fas1-wrap").show(300,function  () {
		     	$(".fas1").focus(function  () {
		     		$(".fas1-fs").click(function  () {
		     			...
		     			var json = {CommentForm};
						$.post("/product/commentadd?token="+token+"&userid="+userid+"",json
						,function(data){
							console.log(data)
						function getLocalTime(nS) {     
						    return new Date(parseInt(nS) * 1000).toLocaleString().substr(5,4)
...
//=============================================点击评论图标==================================================
				...
				$(".fas2-fs").on("click",function  () {
						...
						var CommentForm = {};
		     			...
		     			var json = {CommentForm};
						$.post("/product/commentaddson?token="+token+"&userid="+userid+"",json
						,function(data){
							var iteee=""
