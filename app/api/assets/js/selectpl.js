			...
			this.createAgoCommentHtml = function(){
				...
				$.each(para.agoComment, function(k, v){
					...
					$("body").on("click",".oun-more .oun-font",function  () {
						$(this).parents(".oun-pl-conter").find(".oun-pl-bottom p:gt(2)").show(300)
					})
					//修改楼层数
					var i=1
					$(".oun-pl-conter").each(function  () {
				...
				$(".fas1-wrap").show(300,function  () {
		     	$(".fas1").focus(function  () {
		     		$(".fas1-fs").click(function  () {
						...
						$.post("/recommend/commentadd?token="+token+"&userid="+userid+"",json
						,function(data){
							console.log(data)
						function getLocalTime(nS) {     
						    return new Date(parseInt(nS) * 1000).toLocaleString().substr(5,4)
...
//
//  if ($(".select-wrap").scrollTop()<=0){
//    $(".fas1-wrap").hide()
//  }
//  if (Math.ceil($(".select-wrap").scrollTop()) >= Math.ceil($(document).height() - $(window).height())) {
//	    $(".submit-ll").hide()
//	    sild ()
//  	}
//	});
...
//=============================================点击评论图标==================================================
				...
				$(".fas2-fs").on("click",function  () {
						...
						var CommentForm = {};
		     			...
		     			var json = {CommentForm};
						$.post("/recommend/commentaddson?token="+token+"&userid="+userid+"",json
						,function(data){
							console.log(data)
							var iteee=""
